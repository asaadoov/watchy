...
@section('content')
	<div class="movie-info border-b border-gray-800">
		<div class="container mx-auto px-4 py-16 flex items-center flex-col md:flex-row">
			...
			<div class="md:ml-24 ">
				<h2 class="text-4xl font-semibold">{{ $movie['title'] }} <span class="text-sm text-orange-500 uppercase font-semibold">( Movie )</span></h2>
								...
								<div class="container mx-auto lg:px-32 rounded-lg overflow-y-auto">
									...
									<div class="modal-body px-8 py-8" id="trailer">
										<div class="responsive-container overflow-hidden relative" style="padding-top: 56.25%">
											<iframe id="player" class="responsive-iframe absolute top-0 left-0 w-full h-full" src="https://www.youtube.com/embed/{{ $movie['videos']['results'][0]['key'] }}" style="border:0;" allow="autoplay; encrypted-media" allowfullscreen></iframe>
		...
		<div class="container mx-auto px-4 py-8">
			@if(count($similarMovies) > 0 && $movie['original_language'] != 'ar')
				<div>
					<h4 class="text-3xl text-orange-500 uppercase font-semibold">Similar Movies</h4>
				</div>
				<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8 ">
					@foreach ($similarMovies as $similarMovie)
						<div class="mt-4">
							<a href="{{ route('movie.item', $similarMovie['id']) }}">
								<img src="{{ $similarMovie['poster_path'] }}" alt="{{ $similarMovie['title'] }}" class="hover:opacity-75 transition ease-in-out duration-150">
							</a>
							<a href="{{ route('movie.item', $similarMovie['id']) }}" class="text-sm leading-normal block text-gray-400 hover:text-white mt-1">{{ $similarMovie['title'] }}</a>
						</div>
					@endforeach
				</div>
				@endif

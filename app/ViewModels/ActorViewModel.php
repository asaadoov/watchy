...
class ActorViewModel extends ViewModel
{
    ...
    public function knownFor()
    {
        ...
        return collect($castMovies)->sortByDesc('popularity')->take(5)->map(function($movie){
            ...
            return collect($movie)->merge([
                ...
                'title' => $title,
                'linkToPage' => $movie['media_type'] === 'movie' ? route('movie.item', $movie['id']) : route('tvshow.item', $movie['id']),
                'releaseDate' => $releaseDate,
            ])->only(['poster_path', 'title', 'id', 'releaseDate', 'linkToPage']);
        });
    }
    ...
    public function credits()
    {
        ...
        return collect($castMovies)->map(function($movie){
            ...
            ])->only(['release_date', 'release_year', 'title', 'character', 'linkToPage',]);
